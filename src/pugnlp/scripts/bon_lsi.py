...
def main(Tweet=None):
    ...
    lsi = LsiModel(
        tfidf[bows], num_topics=80, id2word=vocab, extra_samples=100, power_iters=2
    )
    lsi.save(
        "/home/hobs/src/hackor/twote/data/lsi{}x{}x{}.saved".format(
            len(tweets), lsi.num_topics, lsi.num_terms
        )
    )
    topics = lsi[tfidf[bows]]
    topics = pd.DataFrame(
        [dict(d) for d in topics], index=tweets.index, columns=range(80)
    )
